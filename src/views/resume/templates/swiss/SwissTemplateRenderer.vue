  ...
  <TemplatePaper :model="model" variant="modern" class="swiss-renderer">
    ...
    <main v-if="sections.length" class="swiss-renderer__body">
      <section
        ...
        :class="['swiss-renderer__section', `swiss-renderer__section--${section.builtinKey || 'custom'}`]"
      >
        <h2>{{ section.title }}</h2>

        <div v-if="section.builtinKey === 'summary'" class="swiss-renderer__summary">
          <p v-for="paragraph in model.summary" :key="paragraph">{{ paragraph }}</p>
        </div>
        <div v-else-if="section.builtinKey === 'skills'" class="swiss-renderer__skills">
