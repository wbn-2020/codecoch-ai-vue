...
describe('user dashboard visual guardrails', () => {
  ...
  it('uses the dedicated direction D top navigation instead of overriding the legacy dark navigation', () => {
    ...
    expect(userNavigation).toContain("label: '准备'")
    expect(userNavigation).toContain("label: '训练'")
    expect(userNavigation).toContain("label: '面试'")
    expect(userNavigation).toContain("label: '进度'")
    expect(userNavigation).toContain("label: '资源'")
    expect(userNavigation).toContain("label: '成长'")
